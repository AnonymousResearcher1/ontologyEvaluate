...
# load data
graph_path = '../datasets/idoden/graph.txt'
text_path = '../datasets/idoden/data.txt'
f_emb_name = '../datasets/idoden/embed.txt'
f_gain_name = '../datasets/idoden/gain.txt'
f_att_name = '../datasets/idoden/att.txt'
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        model = cane4att.Model(data.num_vocab, data.num_nodes)
        ...
        for i in range(num_batch):
            ...
            # run the graph
            convA, convB, TA, TB, wA, wB = sess.run(
                [model.convA, model.convB, model.N_A, model.N_B,model.attA,model.attB],
                feed_dict=feed_dict)
            for i in range(config.batch_size):
                ...
                embed_pair_att[(node1[i],node2[i])] = wA[i]
                embed_pair_att[(node2[i],node1[i])] = wB[i]
        for i in range(data.num_nodes):
            ...
            if embed[i]:
                ...
                file_emb.write('\n')
        for node1,node2 in embed_pair_att.keys():
            tmp_att = embed_pair_att[(node1,node2)]
            text = ' '.join([str(node1),str(node2)])
            tmp = text + ' ' + ' '.join(map(str,tmp_att))
            file_att.write(tmp + '\n')
